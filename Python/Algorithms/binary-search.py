...
def binary_search_by_recursion(sorted_collection: list[int], item: int, left: int = 0, right: int = -1) -> int:
    if right < 0:
        right += len(sorted_collection) - 1
    if list(sorted_collection) != sorted(sorted_collection):
        raise ValueError("Collection must be sorted")
...
def exponential_search(sorted_collection: list[int], item: int) -> int:
    ...
    last_result = binary_search_by_recursion(sorted_collection, item, left, right)
...
if __name__ == "__main__":
    ...
    result = exponential_search(collection, target)
    if result >= 0:
